...
# Query for Postgres 17 (using prokind='a')
QUERY=$(cat <<EOF
SELECT 
...
while IFS=":" read -r host port; do
    ...
    echo "--- Checking Postgres 17 Host: $host Port: $port ---"
    ...
    for dbname in $DB_LIST; do
        ...
        if [[ -z "$RESULT" ]]; then
            ...
            # Start the file with the connection header
            echo "-- Generated for Postgres 18 Migration" > "$OUTPUT_FILE"
            echo "\\connect $dbname" >> "$OUTPUT_FILE"
            echo "" >> "$OUTPUT_FILE"

            # Process the results and append to the file
            echo "$RESULT" | awk -F "|" '{
